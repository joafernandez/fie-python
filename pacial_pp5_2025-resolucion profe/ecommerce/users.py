# Niveles ---------------------------------------------------------------
class Nivel:
    def __init__(self, nombre, min_puntos, max_puntos, max_productos, saldo_negativo_max):
        self.nombre = nombre
...
class Usuario:
    ...
    def agregar_producto(self, producto):
        ...
        if not self.nivel.permite_agregar(len(self.carrito)):
            raise Exception(f"Nivel {self.nivel.nombre} no permite agregar más productos al carrito")
    ...
    def comprar(self):
        ...
        if not self.nivel.permite_descubierto(saldo_post):
            raise Exception("Saldo insuficiente para completar la compra según su nivel")
